...
def load_wav_to_torch(full_path, tsp=22050):
    ...
    data = trim(data, frame_length=6000, hop_length=200, top_db=35)
    return torch.FloatTensor(data.astype(np.float32)), sampling_rate


def trim(y, frame_length=6000, hop_length=200, top_db=35):
    y, _ = librosa.effects.trim(
        y, frame_length=frame_length, hop_length=hop_length, top_db=top_db)
    return y
  ...
  with open(filename, encoding='utf-8') as f:
    filepaths_and_text = [
        line.strip().split(split)[:-1] + [
            list(map(int, line.strip().split(split)[-1].split(",")))
        ]
        for line in f
    ]
    filepaths_and_text = filepaths_and_text[:max_len]
  return filepaths_and_text
...
def get_hparams(init=True):
  ...
  hparams.data.data_dir = args.data_dir
  return hparams
